document.addEventListener('DOMContentLoaded', () => {
    ...
    function cargarEstadisticasYRanking() {
        ...
        fetch(`http://localhost:3000/api/deportistas/estadisticas/${usuario._id}`)
            ...
            .then(data => {
                ...
                // Filtrar por club seleccionado
                if (clubSeleccionado) {
                    const club = clubesEntrenador.find(c => c._id === clubSeleccionado);
                    const idsClub = club && club.deportistas ? club.deportistas.map(d => d._id) : [];
                    dataFiltrada = dataFiltrada.filter(dep => idsClub.includes(dep.deportista._id));
                }
    ...
    const listaClubesDiv = document.getElementById('listaClubes');
    let clubesEntrenador = [];
    let deportistasEntrenador = [];
    let clubSeleccionado = null;

    // Cargar deportistas asociados (guardar en variable para clubes)
    fetch(`http://localhost:3000/api/deportistas/entrenador/${usuario._id}`)
        .then(res => res.json())
        .then(data => {
            deportistasEntrenador = data || [];
            renderListaDeportistas();
        });
    ...
    formCrearClub.addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch('http://localhost:3000/api/clubes', {
                ...
                body: JSON.stringify({ nombre, entrenadorId: usuario._id })
            });
            if (res.ok) {
                nombreClubInput.value = '';
                await cargarClubes();
            } else {
    ...
    // Cargar clubes del entrenador
    async function cargarClubes() {
        try {
            const res = await fetch(`http://localhost:3000/api/clubes/entrenador/${usuario._id}`);
            clubesEntrenador = await res.json();
            renderListaClubes();
            renderListaDeportistas();
        } catch (err) {
            listaClubesDiv.innerHTML = '<span class="text-danger">Error al cargar clubes</span>';
        }
    }

    // Renderizar lista de clubes y opción de agregar deportistas
    function renderListaClubes() {
        let html = `<button class='btn btn-sm btn-outline-primary mb-2 w-100' id='btnVerTodosClubes'>Ver todos los deportistas</button>`;
        if (!Array.isArray(clubesEntrenador) || clubesEntrenador.length === 0) {
            html += '<span class="text-muted">No tienes clubes creados.</span>';
            listaClubesDiv.innerHTML = html;
            return;
        }
        clubesEntrenador.forEach(club => {
            html += `<div class='mb-2 border rounded p-2 ${clubSeleccionado === club._id ? 'bg-primary text-white' : ''}' style='cursor:pointer;' onclick='seleccionarClub("${club._id}")'>
                <div class='d-flex justify-content-between align-items-center'>
                    <div><strong>${club.nombre}</strong></div>
                    <div class='d-flex gap-2'>
                        <button class='btn btn-sm btn-outline-light' title='Editar' onclick='event.stopPropagation();editarClub("${club._id}", "${club.nombre.replace(/"/g, '&quot;')}")'><i class="bi bi-pencil text-primary"></i></button>
                        <button class='btn btn-sm btn-outline-light' title='Eliminar' onclick='event.stopPropagation();eliminarClub("${club._id}", "${club.nombre.replace(/"/g, '&quot;')}")'><i class="bi bi-trash text-danger"></i></button>
                        <button class='btn btn-sm btn-outline-light' onclick='event.stopPropagation();mostrarAgregarDeportistas("${club._id}")'>Agregar deportistas</button>
                    </div>
                </div>
                <div class='mt-2'><span class='fw-bold'>Miembros:</span> ${club.deportistas && club.deportistas.length > 0 ? club.deportistas.map(d=>d.nombre).join(', ') : '<span class="text-light">Sin deportistas</span>'}</div>
                <div id='agregarDeportistas_${club._id}' class='mt-2' style='display:none;'></div>
            </div>`;
        });
        listaClubesDiv.innerHTML = html;
        document.getElementById('btnVerTodosClubes').onclick = () => {
    ...
    // Nueva función para renderizar la lista de deportistas según el club seleccionado
    function renderListaDeportistas() {
        listaDeportistas.innerHTML = '';
        let deportistasAMostrar = [];
        if (clubSeleccionado) {
            const club = clubesEntrenador.find(c => c._id === clubSeleccionado);
            deportistasAMostrar = club && club.deportistas ? club.deportistas : [];
        } else {
            deportistasAMostrar = deportistasEntrenador;
        }
        if (!Array.isArray(deportistasAMostrar) || deportistasAMostrar.length === 0) {
            listaDeportistas.innerHTML = '<li class="list-group-item text-center text-muted">No hay deportistas para mostrar.</li>';
        } else {
            deportistasAMostrar.forEach(dep => {
                const li = document.createElement('li');
                li.className = 'list-group-item deportista-item';
                li.style.cursor = 'pointer';
                li.innerHTML = `<strong>${dep.nombre}</strong> <br><span class='text-muted small'>${dep.correo}</span>`;
    ...
    // Función global para seleccionar club
    window.seleccionarClub = function(clubId) {
        clubSeleccionado = clubId;
        renderListaClubes();
        renderListaDeportistas();
    };

    // Mostrar formulario para agregar deportistas a un club
    window.mostrarAgregarDeportistas = function(clubId) {
        const div = document.getElementById('agregarDeportistas_' + clubId);
        if (!div) return;
        // Mostrar select múltiple con los deportistas
        let html = `<form onsubmit='return false;' id='formAddDeportistas_${clubId}'>`;
        html += `<select multiple class='form-select mb-2' id='selectDeportistas_${clubId}'>`;
        deportistasEntrenador.forEach(dep => {
            html += `<option value='${dep._id}'>${dep.nombre} (${dep.correo})</option>`;
        });
        html += `</select>`;
        html += `<button type='submit' class='btn btn-sm btn-success'>Agregar seleccionados</button>`;
        html += ` <button type='button' class='btn btn-sm btn-secondary' onclick='ocultarAgregarDeportistas("${clubId}")'>Cancelar</button>`;
        html += `</form>`;
        div.innerHTML = html;
        div.style.display = 'block';
        // Manejar submit
        document.getElementById('formAddDeportistas_' + clubId).onsubmit = async function() {
            const select = document.getElementById('selectDeportistas_' + clubId);
            const seleccionados = Array.from(select.selectedOptions).map(opt => opt.value);
            if (seleccionados.length === 0) return;
            try {
                const res = await fetch('http://localhost:3000/api/clubes/agregar-deportistas', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ clubId, deportistas: seleccionados })
                });
                if (res.ok) {
                    await cargarClubes();
                } else {
                    alert('Error al agregar deportistas');
                }
            } catch (err) {
                alert('Error de conexión');
            }
    ...
    window.ocultarAgregarDeportistas = function(clubId) {
        const div = document.getElementById('agregarDeportistas_' + clubId);
        if (div) div.style.display = 'none';
    };

    // Eliminar club con confirmación
    window.eliminarClub = async function(clubId, clubNombre) {
        if (!confirm(`¿Seguro que deseas eliminar el club "${clubNombre}"? Esta acción no se puede deshacer.`)) return;
        try {
            const res = await fetch(`http://localhost:3000/api/clubes/${clubId}`, {
                method: 'DELETE'
            });
            if (res.ok) {
                if (clubSeleccionado === clubId) clubSeleccionado = null;
                await cargarClubes();
            } else {
    ...
    // Editar club (inline)
    window.editarClub = function(clubId, clubNombre) {
        const div = document.querySelector(`[onclick*='seleccionarClub(\"${clubId}\")']`);
        if (!div) return;
        div.innerHTML = `<form onsubmit='return false;' id='formEditarClub_${clubId}' class='d-flex gap-2 align-items-center'>
            <input type='text' class='form-control form-control-sm' id='inputEditarClub_${clubId}' value='${clubNombre}' required style='max-width: 150px;'>
            <button type='submit' class='btn btn-sm btn-success'><i class='bi bi-check'></i></button>
            <button type='button' class='btn btn-sm btn-secondary' onclick='cancelarEditarClub()'><i class='bi bi-x'></i></button>
        </form>`;
        document.getElementById(`formEditarClub_${clubId}`).onsubmit = async function() {
            const nuevoNombre = document.getElementById(`inputEditarClub_${clubId}`).value.trim();
    ...
    window.cancelarEditarClub = function() {
        renderListaClubes();
    };
